...
public class GroupsController {
    ...
    public ResponseEntity<ResponseWrapper<GroupDTO>> createGroup(@Valid @RequestBody GroupInsertDTO groupInsertDTO) {
        Result<GroupRelationshipsDTO> relationshipsResult = groupService.getGroupRelationshipsData(groupInsertDTO);
        ...
        Result<Void> scheduleResult = groupService.validateGroupSchedule(groupInsertDTO.getClassroom(), groupInsertDTO.getSchedule());
        if (!scheduleResult.isSuccess()) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(ResponseWrapper.conflict(scheduleResult.getErrorMessage()));
        }
